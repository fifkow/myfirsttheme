<?php
/**
 * Template Name: Elementor Full Width
 *
 * This template removes the theme header and footer,
 * allowing Elementor to build the entire page.
 *
 * @package Neve_Lite
 * @since 1.0.0
 */

?><!doctype html>
<html <?php language_attributes(); ?>>
<head>
    <meta charset="<?php bloginfo( 'charset' ); ?>">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="profile" href="https://gmpg.org/xfn/11">
    
    <?php wp_head(); ?>
</head>

<body <?php body_class( 'nv-elementor-full-width-page' ); ?>>
<?php wp_body_open(); ?>

<a class="skip-link screen-reader-text" href="#primary">
    <?php esc_html_e( 'Skip to content', 'neve-lite' ); ?>
</a>

<?php
/**
 * Hook: neve_lite_before_header
 *
 * @since 1.0.0
 */
do_action( 'neve_lite_before_header' );
?>

<header id="masthead" class="nv-header nv-header-default">
    <div class="nv-container">
        <div class="nv-header-inner">
            
            <div class="nv-header-left">
                <?php
                // Site Logo
                neve_lite_site_logo();
                ?>
            </div>
            
            <div class="nv-header-center">
                <?php
                // Primary Navigation
                neve_lite_primary_navigation();
                ?>
            </div>
            
            <div class="nv-header-right">
                <?php
                // WooCommerce Header Cart
                if ( class_exists( 'WooCommerce' ) ) {
                    neve_lite_header_cart();
                }
                
                // Mobile Menu Toggle
                neve_lite_mobile_menu_toggle();
                ?>
            </div>
            
        </div>
    </div>
</header>

<?php
/**
 * Hook: neve_lite_after_header
 *
 * @since 1.0.0
 */
do_action( 'neve_lite_after_header' );
?>

<div id="page" class="nv-site">
    
    <div id="content" class="nv-site-content nv-elementor-content">
        
        <main id="primary" class="nv-content-area nv-elementor-full-width">
            
            <?php
            while ( have_posts() ) :
                the_post();
                
                the_content();
                
            endwhile;
            ?>
            
        </main><!-- #primary -->
        
    </div><!-- #content -->
    
</div><!-- #page -->

<?php wp_footer(); ?>

</body>
</html>
